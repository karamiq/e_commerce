...
export class RolesService {
  ...
  async create(createRoleDto: CreateRoleDto): Promise<Roles> {
    ...
    if (permissionIds && permissionIds.length > 0) {
      role.permissions = await this.validateAndFetchPermissions(permissionIds);
    }
  ...
  async addPermissionsToRole(roleId: string, permissionIds: string[]): Promise<Roles> {
    ...
    const newPermissions = await this.validateAndFetchPermissions(permissionIds);
  ...
  private async validateAndFetchPermissions(permissionIds: string[]): Promise<Permissions[]> {
    if (!permissionIds || permissionIds.length === 0) {
      return [];
    }
