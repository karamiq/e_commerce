...
export class RefreshTokenGuard extends AuthGuard('jwt-refresh') {
  ...
  private async removeRefreshToken(context: ExecutionContext) {
    try {
      ...
      const decodedToken = await this.tokensProvider.decodeRefreshToken(
        request.headers['authorization']?.split(' ')[1],
      );
      const refreshTokenEntity = await this.userRefreshTokenRepository.findOne({
        where: { user: { id: decodedToken.sub } },
      });
      console.log(
        'has the refresh token expired? ',
        refreshTokenEntity && refreshTokenEntity.expiresAt < new Date(),
      );
